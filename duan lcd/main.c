...
volatile unsigned int time,counter;
volatile unsigned int second1,second2,second3,second4;
...
void dealwithnumber()
{
	second1 = counter % 60 % 10 ;
	second2 = counter % 60 / 10 ;
	second3 = counter / 60 % 10 ;
	second4 = counter / 60 / 10 ;
}
...
__interrupt void secondwatch()
{
	counter++;
	dealwithnumber();
	LCDMEM[5] = number[second1];
	LCDMEM[4] = number[second2];
	LCDMEM[3] = number[second3];
	LCDMEM[2] = number[second4];
}
